...
We then patch the top-$k$ heads simultaneously, replacing their outputs at the newline with those from the corrupt forward pass, and measure the resulting corrupt rhyme rate. Figure~\ref{fig:topk-heads}b shows that $k=1,2,3$ all yield near-zero rates, so the three highest-attending heads are jointly insufficient to carry the full phonological signal. At $k=5$ the rate jumps to 0.46, non-overlapping with $k=3$ and comparable to the full-residual-stream peak. The ratio of top-5 head patching to the full-residual peak (joint cluster bootstrap over the same five prompt pairs) is 0.71 [0.46, 0.88], confirming that this small subset of heads carries most---though, given the width of the interval, possibly not all---of the effect attainable from the full residual stream. Adding further heads does not improve the rate, which plateaus at $k=10$.

These results reveal a sparse but distributed implementation of the handoff: five attention heads spanning layers 28--34 jointly account for the bulk of the rhyme information routed to the newline, while individual heads and smaller subsets are insufficient. The top five heads are (layer 30, head 4), (layer 28, head 14), (layer 28, head 15), (layer 30, head 5), and (layer 28, head 29). The analogous top-$k$ MLP patches at the same position yield a corrupt rhyme rate of zero at every $k$ tested (Wilson 95\% upper bound $\leq 0.04$, $N{=}100$), well below the head-patching effect at $k=5$, confirming that the handoff is mediated by attention rather than feed-forward computation.
...
Beyond the layer-level analysis, we localize the handoff in Gemma-3-27B to a sparse circuit of five attention heads in layers 28--34, identified via attention weight ranking and simultaneous head patching. These five heads recover 0.71 [95\% CI 0.46, 0.88] of the effect achievable by full residual-stream patching, while MLP patching at the same position is at noise (Wilson 95\% upper bound $\leq 0.04$) and single-head patching produces no measurable signal. The CI on the head-patching ratio is wide because the unit of independence is the prompt pair and we have only five, but its lower bound is well above the MLP upper bound, so the qualitative claim is robust. This circuit-level specificity suggests that planning site formation, when it occurs, is a structured computational phenomenon rather than a diffuse property of many model components.
