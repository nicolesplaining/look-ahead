...
Beyond the layer-level analysis, we localize the handoff in Gemma-3-27B to a sparse circuit of five attention heads in layers 28--34. Two-stage path patching~\citep{goldowsky2023localizing,wang2022ioi} establishes that these five heads recover 88\% of the corrupt rhyme rate achievable by overwriting the entire newline residual at the best layer, while MLP patches at the newline are at noise and single-head patches produce no measurable signal. Crucially, the K-sweep \emph{peaks at five heads and declines} as more heads are added, and random and comma-control head sets remain at zero across all $k$, indicating that the five-head set is not merely sufficient but specifically the routing circuit. This suggests that planning site formation, when it occurs, is a structured computational phenomenon rather than a diffuse property of many model components.
