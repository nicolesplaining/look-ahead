"""
Activation-patching baseline experiments for Qwen3-32B.
...
Key position: i=-1 (last word token for Qwen3 due to tokenization).
Sweeps all layers, 4 prompt pairs, N=20 samples each.

Results saved to results/QWEN3_BASELINE/
"""
...
PATCH_OFFSET = -1   # i=-1 is the last word token for Qwen3
...
def run_all():
    ...
    # Pre-cache donor activations at the same absolute token position as the patch
    ...
    for pair in PROMPT_PAIRS:
        ...
        # Donor patch vecs (use pre-cached donor, broadcast to each layer)
        donor_vecs = donor_cache  # list of (1, d_model) tensors
