"""
Aggregate activation-patching experiment across 4 prompt pairs — Qwen3-32B.
...
def run_pair(model, tokenizer, pair: dict, results_dir: str) -> dict:
    ...
    # Sampling baseline: unpatched clean run (shared across all positions)
    ...
    # Save greedy + baseline to pair-level JSON
    pair_meta = {
        "pair_id":             pair_id,
        "clean_prompt":        clean_prompt,
        "corrupt_prompt":      corrupt_prompt,
...
def compute_aggregate(all_position_exports: list) -> dict:
    """
    all_position_exports: list of dicts (one per pair), each mapping pos_id -> export.
    Returns: dict mapping pos_id -> list of per-layer aggregate stats.
    """
