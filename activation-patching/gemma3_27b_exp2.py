"""
Activation patching on Gemma-3-27B-IT with fright/fear prompt pair.

Patch direction: corrupt → clean
  - Corrupt prompt: "...sense of fear,\nand hoped that"
  - Clean prompt:   "...sense of fright,\nand hoped that"
  - Run corrupt prompt, patch in clean activations at a single token position.
  - Sweep all layers.

Four experiments, one per token position:
  1. newline (last '\n' in corrupt prompt)
  2. 'and'
  3. ' hoped'
  4. ' that'
...
EXPERIMENTS = [
    {"experiment_id": "exp_fear_newline", "selector": {"kind": "newline"}},
    {"experiment_id": "exp_fear_and",     "selector": {"kind": "token_text", "token_text": "and"}},
    {"experiment_id": "exp_fear_hoped",   "selector": {"kind": "token_text", "token_text": " hoped"}},
    {"experiment_id": "exp_fear_that",    "selector": {"kind": "token_text", "token_text": " that"}},
]
...
def find_patch_pos(tokenizer, prompt: str, selector: dict) -> tuple:
    """
    Returns (patch_pos: int, patch_label: str).

    selector kinds:
      {"kind": "newline"}
          → last token that covers a '\n' character in the prompt
      {"kind": "token_text", "token_text": str}
          → last contiguous token span that decodes to token_text; returns
            the position of the final token in that span
    """
    ...
    if selector["kind"] == "token_text":
        ...
        # Find last occurrence by scanning token spans
        ...
        if last_match_end is not None:
            ...
            return patch_pos, patch_label
        # Fallback: search by decoded text of each token
...
def run_single_experiment(
    ...
    # --- Find patch position in both prompts ---
    ...
    # Print token context around patch position
